...
func (s *Server) callbackHandler(c *gin.Context) {
	...
	user, isNewUser, err := GetUserByOauth(ctx, platform, code, &parameterCarrier)
	if err != nil {
		status, codeMsg := mapUserCenterError(err)
		response.HandleError(c, status, codeMsg, fmt.Errorf("%s: %v", errs.ErrInfoQueryUserInfo, err))
		return
	}
...
func GetUserByOauth(ctx context.Context, typ, code string, parm *ParameterCarrier) (*repository.AuthUser, bool, error) {
...
func fetchUserByIdentityChain(ctx context.Context, accessToken string) (*repository.AuthUser, bool, error) {
	...
	// github identity row → GithubID / GithubName (decision 6 field mapping)
	if csUser.SubjectID != "" {
...
// normalizePhone strips the +86 country code prefix (kept from the old
// GetUserInfo path).
...
// mapUserCenterError maps a login-chain RPC failure to an HTTP status + error
// code for the callback handlers (decision 2 / §2.1).
func mapUserCenterError(err error) (int, string) {
	switch {
	case errors.Is(err, usercenter.ErrInvalidIdentity):
		return http.StatusUnauthorized, errs.ErrInvalidIdentity
	case errors.Is(err, usercenter.ErrExplicitlyUnbound):
		return http.StatusConflict, errs.ErrInvalidIdentity
	default:
		return http.StatusServiceUnavailable, errs.ErrServiceUnavailable
	}
}
...
// handleInviterCodeValidation validates the inviter code and records the
// inviter on a cs-user-confirmed new user. Existing users never reach here
// (decision 9): they ignore the invite code and complete login normally.
